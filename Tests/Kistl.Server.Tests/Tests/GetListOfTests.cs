...
namespace Kistl.Server.Tests
{
    ...
    public class GetListOfTests
    {
        ...
        public void GetListOf()
        {
            ...
            {
                var prj = ctx.GetQuery<Kistl.App.Projekte.Projekt>().Where(o => o.Name == "Kistl").First();
                var list = prj.Tasks;
        ...
        public void GetListOf_Twice()
        {
            using (Kistl.API.IKistlContext ctx = KistlContext.GetContext())
            {
                var prj1 = ctx.GetQuery<Kistl.App.Projekte.Projekt>().Where(o => o.Name == "Kistl").First();
                var list1 = prj1.Tasks;
                Assert.That(list1.Count, Is.GreaterThan(0));

                var prj2 = ctx.GetQuery<Kistl.App.Projekte.Projekt>().Where(o => o.Name == "Kistl").First();
        ...
        public void GetObject_GetListOf()
        {
            using (Kistl.API.IKistlContext ctx = KistlContext.GetContext())
            {
                ...
                var prj = ctx.GetQuery<Kistl.App.Projekte.Projekt>().Where(p => p.Name == "Kistl").First();
