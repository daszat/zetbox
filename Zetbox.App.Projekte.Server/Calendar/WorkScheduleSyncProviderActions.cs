...
namespace Zetbox.App.Calendar
{
    ...
    {
        [Invocation]
        public static async System.Threading.Tasks.Task PerformSync(WorkScheduleSyncProvider obj)
        {
            ...
            while (dt <= nextYear)
            {
                foreach(var rule in await rules.Where(async r => await r.AppliesTo(dt)))
                {
